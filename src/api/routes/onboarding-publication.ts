...
import {getOnboardingScheduling,publishOnboardingKickoff,publishOnboardingCheckin,enableOnboardingFollowups} from "../../db/onboarding-publication-repo";
export interface OnboardingPublicationDeps {requireAuth:MiddlewareHandler<AuthEnv>;get:typeof getOnboardingScheduling;publish:typeof publishOnboardingKickoff;publishCheckin:typeof publishOnboardingCheckin;enable:typeof enableOnboardingFollowups;}
const defaults:OnboardingPublicationDeps={requireAuth:requireSession,get:getOnboardingScheduling,publish:publishOnboardingKickoff,publishCheckin:publishOnboardingCheckin,enable:enableOnboardingFollowups};
export function createOnboardingPublicationRoutes(deps:OnboardingPublicationDeps=defaults) {
  ...
  const mutate=(action:"publish-kickoff"|"publish-checkin"|"enable-followups"):Handler<AuthEnv>=>async c=>{
    const user=c.get("user");if(!user.workspaceId||!user.workspaceRole)return c.json({error:"workspace_not_found"},404);
    if(action!=="enable-followups" && !["admin","owner"].includes(user.workspaceRole))return c.json({error:"forbidden"},403);
    if(!z.string().uuid().safeParse(c.req.param("id")).success)return c.json({error:"invalid_id"},400);
    const body=await c.req.json().catch(()=>null);
    const parsed=action!=="enable-followups"?publishOnboardingInput.safeParse(body):enableFollowupsInput.safeParse(body);
    ...
    const result=action==="publish-kickoff"?await deps.publish(user.workspaceId,actor,c.req.param("id")!,publishOnboardingInput.parse(body)):
      action==="publish-checkin"?await deps.publishCheckin(user.workspaceId,actor,c.req.param("id")!,publishOnboardingInput.parse(body)):
  ...
  router.post("/api/me/engagements/:id/onboarding-scheduling/publish-checkin",mutate("publish-checkin"));
