...
test("publication and cadence activation authenticate, scope the workspace and reject unreviewed extra input",async()=>{
 ...
 const deps:OnboardingPublicationDeps={requireAuth:auth,get:async()=>({kind:"not_found"}),publish:async(workspaceId,actor,engagementId,input)=>{called={workspaceId,actor,engagementId,input};return {kind:"blocked",issues:["rollout_not_enabled"]};},publishCheckin:async()=>({kind:"forbidden"}),enable:async()=>({kind:"forbidden"})};
