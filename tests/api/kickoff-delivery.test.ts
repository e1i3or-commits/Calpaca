...
function deps(role:"admin"|"member"|null):KickoffDeliveryApiDeps {
 ...
 return {requireAuth,secret:()=>"synthetic-feedback-secret",report:async()=>({checkedAt:new Date().toISOString(),workerStale:true,lastSweepAt:null,attention:0,reservationAttention:0,reservationPending:0,reservationOverdue:0,schedulerStale:false,schedulerLastSweepAt:null,overdue:0,deliveries:[]}),receipt:async()=>({kind:"recorded"}),retry:async()=>({kind:"queued"})};
}
