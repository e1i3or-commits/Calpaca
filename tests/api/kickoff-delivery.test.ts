...
function deps(role:"admin"|"member"|null):KickoffDeliveryApiDeps {
 ...
 return {requireAuth,secret:()=>"synthetic-feedback-secret",report:async()=>({checkedAt:new Date().toISOString(),workerStale:true,lastSweepAt:null,attention:0,overdue:0,deliveries:[]}),receipt:async()=>({kind:"recorded"}),retry:async()=>({kind:"queued"})};
}
