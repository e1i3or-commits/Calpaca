...
const messages: Record<string,string> = {
  calendar_reconciliation_blocked: "The calendar change could not be applied safely. Existing dates are preserved. Check team availability and outstanding invitation issues, then preview again.",
  ...
  return <section className="mt-6 rounded-lg border border-border p-4" aria-labelledby="followup-schedule-title">
    ...
    {!snapshot?<p role="status" className="mt-3 text-sm">{error?"Schedule unavailable. Use Refresh schedule to retry.":"Loading follow-up schedule…"}</p>:<>
      ...
      {pending&&<div className="mt-5 rounded-lg border border-border p-4" aria-label="Schedule change preview">
        ...
        <p className="mt-1 text-sm text-muted-foreground">{snapshot.deliveryState==="reservations_present"?"Saving moves existing invitations or queues cancellations for the changes below. Delivery remains pending until verified. New or resumed dates require reservation before invitation.":pending.preview.status==="ended"?"Ending cancels future draft dates and preserves history. An ended schedule cannot be resumed.":pending.preview.status==="paused"?"Future draft dates will be paused. No invitations will be sent.":"Saving updates planned dates only. No invitations will be sent."}</p>
        ...
        <ul className="mt-3 divide-y divide-border text-sm">{pending.preview.changes.map(row=><li key={row.id??`new-${row.position}`} className="py-2"><span className="font-medium capitalize">{row.action}</span>{row.action==="resume"?" date after cancellation":row.id&&schedule?.occurrences.find(item=>item.id===row.id)?.bookingId?" existing invitation":" draft date"} · {format(row.startsAt,pending.preview.rule?.timezone)}{row.previousStartsAt&&row.previousStartsAt!==row.startsAt&&<span className="text-muted-foreground"> · Previously {format(row.previousStartsAt,schedule?.rule.timezone)}</span>}{row.exception&&" · Individual exception"}</li>)}</ul>
